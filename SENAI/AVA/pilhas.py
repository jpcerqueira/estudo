...
def desempilhar():
    global topo # permite que modifique a variável global, onde está no escopo global, dentro da função
    if not vazia():
        nome = pilha[topo]
...
def listar():
    for item in pilha[::-1]: # percorre a 'lista' ao contrário através do slicing, onde veremos do topo até a base da pilha, poderia usar o 'reversed(pilha):'
        print(item)
